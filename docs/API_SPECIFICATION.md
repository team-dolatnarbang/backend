...
| 헌화·적립 | 동백 1송이 + 메시지. **유저 결제 없음.** 동백 1송이당 **1,000원 상당**을 시스템에서만 합산(포인트·적립액 형태). 실제 기부 실행은 **제휴 기업 등 사회적 약속**으로 별도 처리. |
...
- **지도 핑:** 순서 잠금이 서버와 맞아야 하면 헤더 `X-Session-Id: <uuid>` 또는 쿠키 기반 **익명 세션**으로 `complete-listen` 진행 상태를 추적할 수 있다. (선택이며, 클라이언트만으로 순서를 관리해도 된다.)
...
쿼리: (선택) `sessionId` 또는 세션 쿠키로 잠금 해제 여부 판단.
...
## 5. 헌화 메시지·동백 적립 (결제·PG 없음)

일반 유저는 **계정·로그인 없이** 헌화한다. 표시 이름은 **`nickname`** 한 필드만 사용한다. **동일 닉네임 다수 허용**(UNIQUE 제약 없음).

### 5.0 적립 규칙 (시스템)

- **동백 1송이 = 적립 단위 1건 = 1,000원 상당**으로 서버가 기록한다. (UI 카피: “온기 보내기 한 번당 동백 1송이, 1,000원이 함께 쌓여요” 등.)
...
```json
{
  "tributeId": "uuid",
  "camelliaCount": 1,
  "pledgedAmountWon": 1000,
  "createdAt": "2026-04-02T12:00:00+09:00"
}
```

- `camelliaCount`: 이번 요청으로 반영된 동백 송이 수(기본 **1**).
- `pledgedAmountWon`: 이번 요청으로 시스템에 **적립된 원화 상당액**(기본 **1000**). 향후 “동백 N송이” 이벤트 시 `camelliaCount`와 곱해 일치시키면 된다.

### 5.2 헌화 메시지 목록 (공개 피드)

**`GET /tributes`**

쿼리: `page`, `size`, 선택 `siteId`.
...
- `camelliaTotal`: 누적 동백 송이 합(기본적으로 `participantCount`와 같음. 1회당 1송이일 때).
...
2. **서버 연동(선택):** 업로드·조회 시 동일한 **`X-Session-Id`** 헤더를 보내도록 하고, 서버는 해당 기록을 세션과 연결해 저장한다. 클라이언트는 새로고침 후에도 세션 ID만 유지되면 §7.1a `GET /contributions` 로 본인 업로드 목록을 받아 `contributionId` 를 복구할 수 있다.
...
로그인 없이 “내가 올린 목록”을 복구하려면 §7.0 항목 2와 함께 쓴다.

**`GET /contributions`**

**요청 헤더 (필수):** `X-Session-Id: <uuid>` — 업로드 시 `POST /contributions/audio` 에 보냈던 값과 동일해야 한다.
...
- 구현하지 않을 경우: §7.0 항목 1(**localStorage**)만으로도 시연 가능.
...
| 엔티티 | 주요 필드 |
|--------|-----------|
| `Site` | id, order, name, lat, lng, imageUrls[], descriptionText, narrationAudioUrl |
| `ElderContribution` | id, siteId, contributorName, rawAudioUrl, rawTranscript, correctedText, ttsAudioUrl, status |
| `Tribute` | id, siteId, message, nickname(중복 가능), camelliaCount(기본 1), pledgedAmountWon(기본 1000), createdAt |
| `UserProgress` | sessionId, completedSiteOrders[], lastCompletedAt |
